...
- **The quantum Dirac fermion — spin–statistics.**  Spin-½ is realised
  ...
  *also* measured (`n3_exchange_statistics.py`, 3/3): braiding two identical
  ...
  That geometric ``−1`` is now shown to be **dynamically meaningful**, not an
  imprint artefact (`n3_dynamical_braid.py`, 3/3): a ``½`` pinned to a κ well and
  co-evolved under the CGL keeps its winding *and* its ``−1`` holonomy (D2), and
  is *transported* by a moving well with its winding conserved — **adiabatically,
  with a measured speed limit** (D1: below a critical well speed the winding
  follows, above it the vacated core heals before the winding migrates, because
  the κ well pins *amplitude*, not phase).  Braiding two co-evolved ``½``'s
  ...
  **gauge-invariant** to machine precision (G3, which a fixed phase template is
  not).  The vortex is a genuine gauged particle.  What remains is now sharply
  the *quantum* step: **Fock-space anticommutation** (the operator algebra
  ``{ψ, ψ†}`` and a many-body Pauli principle) and the **Aharonov–Bohm /
  Chern–Simons statistical phase** — the flux attachment by which this gauge
  field turns the gauged ½-vortex's topological exchange sign into a genuine
  dynamical *fermion* statistic.  A classical, self-consistent gauge theory is in
  hand; second quantisation is the standing frontier.
