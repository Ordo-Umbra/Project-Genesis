...
*The chronological record: roughly a hundred experiments, each with its
pre-registered question, its measurement and its verdict — including the ones
that failed. This was the bulk of the README until it stopped being readable at
2100 lines, at which point it was doing the opposite of its job.*

*Nothing here is deleted or summarised away. It is moved, because a landing page
and a lab notebook are different documents with different readers. For the
theory as a whole see [`The_Principle.md`](The_Principle.md); for a single result
that stands entirely alone see [`Junction_Selection.md`](Junction_Selection.md).*

*Entries are in the order they were run, so later ones sometimes correct earlier
ones. Where that happens the original is kept with the correction beside it —
the record of having been wrong is part of what makes the rest checkable.*
...
|-----------|---------|-----|
| "Sometimes a 3-D run doesn't die" is a real effect, not memorable-sample bias | **Supported, with the predicted mechanism wrong** — the hazard falls 2.5× (CI [0.227, 0.691]) and goes to zero after step 720; the ending is foreseeable at step 60 from the largest sector's share (fresh-ensemble AUC 0.801, p = 0.0005), *not* from palette imbalance as predicted | [Survival](#findings-so-far) |
...
| Capacity κ drives selection toward `N⋆ = 3` | **Not supported** — measured directly in `n3_capacity_gating`: P=3 peaks at every capacity level including *no capacity field at all*, and the margin is capacity-invariant among valid arms. The selection is geometric (Plateau's law), not scarcity-gated. Previously recorded as **conditional / transient** — a 3-well S-optimal band appears *while the field is actively coarsening*, but in steady state selection runs to more sectors; traced to how ΔI is measured (below) | [Phase diagram](#the-phase-diagram-of-n3-selection) |
...
### The self-sustained vortex: topology kept by the dynamics — 2/3 (an honest boundary)

That "deeper version" is the next test. `experiments/n3_emergent_vortex.py` removes the re-imprinting entirely: the vortices are seeded **once** and the chiral field then co-evolves under the κ-detuned CGL alone (`evolve_seeded_field`, or `evolve_vortex_molecule(reimprint=False)`) — a fully self-sustained field, kept or lost by the dynamics and anchored only by the κ wells. What survives that, and what does not, is the point. **2/3 registered predictions land — and the miss is the informative one:**

- **E1 ✓ The winding is a dynamically-conserved topological charge.** With no re-imprinting, the integer winding enclosing each κ well is preserved for same-sign vortices across the whole run (held at −1.0/−1.0 over 2000 steps, max drift < 0.1), and stays integer even from a **noisy seed** (15% complex noise) — quantisation the dynamics enforces, not a value imposed each step. The tracker-free `winding_number` counts the enclosed integer directly.
- **E2 ✓ Topological selection, and matter-pinning.** Like charges *survive and stay bound to matter*: a same-sign pair keeps |w| = 1 at each well, the cores persist (min|ψ| = 0.02), the field holds real angular momentum (L = 13454 → 6699, the seed's sign), and — released as a free molecule — the winding around each *moving* mass stays ±1 (min|w| = 1.0/1.0: the self-sustained vortex tracks its form, emergent pinning). Unlike charges *annihilate*: a vortex–antivortex pair unwinds to w = 0, the amplitude heals (min|ψ| → 0.57), and L → 0. A physical, dynamically-enforced sign rule — not an assigned label.
- **E3 ✗ But the self-sustained field does NOT reproduce the strong spin** (pre-registered at the pinned run's bar |Ω| ≥ 0.004). Released with the vortices seeded once and never re-imprinted, the molecule's late spin is sign-locked to the winding and above the achiral baseline — late Ω = +0.0002/−0.0000/−0.0017 across q = −1/0/+1 — yet an **order of magnitude weaker** than the pinned mode's +0.0095 at the same operating point, while the field's L drains (13454 → 6395). Without the re-imprinting that re-sharpens the circulation each step, the strong persistent orbital spin does not hold.

Reproduce with `python experiments/n3_emergent_vortex.py` (≈ 18 minutes; `--quick` for a smoke run); self-sustained checks (winding conserved & integer with no re-imprint, vortex–antivortex annihilation with healing, the emergent vortex tracks its moving mass) in `tests/test_vortex_chiral.py`.

**Honest scope:** this is the deeper, self-sustained version of the vorticity-bearing field — seeded once, kept only by the κ-detuned CGL (no re-imprinting). Its topology is real and robust (E1) and it binds to matter with the correct sign rule (E2) — the honest core of a *quantised, matter-bound* spin-analog. What it does **not** do (E3, a pre-registered negative scored against the pinned run's bar) is hold the *strong* orbital spin: without the re-imprinting that re-sharpens the circulation, the field's angular momentum drains and the residual torque is an order weaker (though still sign-locked). So the pinned mode's strong spin was, in part, the imprinting doing work; a self-sustained *strong* orbital drive is the next rung (closed below). One operating point (the derived exclusion floor), equal masses, λ = 0 (the purest topological test), a torus (net winding compensated by the dynamics). The small-lattice caveat: at 48² the 9-wide molecule is too cramped and the like-charge pair loses its winding by ~600 steps — the pinning is a large-box (96²) result.

### The self-sustained *strong* drive: the field's precession regenerates the spin — 3/3

The emergent test's one negative (E3) had a clear diagnosis: at λ = 0 the self-sustained circulation *drains* without the re-imprinting that re-sharpens it. `experiments/n3_driven_vortex.py` supplies the missing regenerator — the field's **own precession** (the CGL twist λ ≠ 0, which turns each vortex into a spiral source that continuously renews its azimuthal current). Everything stays self-sustained: the vortices are seeded **once** and the field co-evolves under `step_chiral_detuned` (`evolve_vortex_molecule(reimprint=False, chiral_lambda=λ)`) — no re-imprinting, no imposed rotation, no `circulation_coupling`. **3/3 registered predictions land — E3 closed:**

- **D1 ✓ The precession drives a strong, self-sustained spin.** With the field's own twist (λ > 0) the emergent molecule holds a persistent spin at the *pinned* strength and beyond, growing with λ: |Ω| = 0.0095/0.0189/0.0218 (q=+1) and 0.0090/0.0178/0.0185 (q=−1) across λ = 0.1/0.2/0.3 — versus the λ = 0 emergent value 0.0017 (an order weaker) and the pinned reference 0.0095. The winding is preserved (min|w| = 1.0, still self-sustained) and every run bound. The strong spin the λ = 0 field could not hold is regenerated by the field's own precession.
- **D2 ✓ The vortex sets the handedness; the precession sets the strength.** At a matched drive, +q and −q spin *oppositely* (sign Ω = sign q, independent of the sign of λ), while |Ω| scales with |λ| and |Ω(+q)| ≈ |Ω(−q)|. Two independent knobs: the topological charge picks the rotation sense, the field's precession sets how fast.
- **D3 ✓ Both ingredients are necessary.** A vortex *without* the precession is the weak, draining λ = 0 spin (0.0017); a precession *without* a vortex carries no torque at all — the phase-current force on a winding-0 field is **exactly 0.00** (a uniformly precessing bulk has no mechanical current, the two-field C2 result). Only the vortex *and* the precession together give the strong sustained spin.

Reproduce with `python experiments/n3_driven_vortex.py` (≈ 25 minutes; `--quick` for a smoke run); driven checks (the precession gives a strong self-sustained spin, +q/−q spin oppositely, a winding-0 field carries no phase force) in `tests/test_vortex_chiral.py`.

**Honest scope:** this closes the emergent test's E3 negative — the self-sustained field (seeded once, co-evolved, **no** re-imprinting) now holds the strong spin, its circulation regenerated by the field's own precession λ rather than by re-sharpening. The handedness is the vortex charge's, the strength the precession's. The strong-spin state lives in a **chirality-compatibility window**: a twist of the wrong sign can unwind the vortex (runs with lost winding are dropped from the stable set and reported). The CGL is a driven, non-equilibrium medium — λ inputs energy, so the recorded energy is not a Lyapunov function (that drive is exactly what a persistent spin costs); the field's L relaxes from the sharp seed to a sustained fraction (the *sustained* spin is the physics, not the imprint transient). One operating point (the derived exclusion floor), equal masses, a torus.

### 3D spin: the vortex is a line, its spin an axial vector — 3/3 (field-level)

Everything above was 2D, where spin lived on a *point* vortex and its angular momentum was a scalar — a sign. In 3D a complex field ψ: ℝ³ → ℂ vanishes generically on **lines** (codimension 2), so the defect is a vortex **line**, and the field's angular momentum L = Σ (x − c) × j is a genuine **3-vector aligned with the line**. `experiments/n3_vortex_3d.py` / `project_genesis/vortex_chiral_3d.py` measure this at field level (no molecule dynamics yet). **3/3 registered predictions land:**

- **V1 ✓ Spin is a quantised axial vector, aligned with the line.** Measured over a centred sphere, L points along the line for *every* orientation — the coordinate axes, the face-diagonal (1,1,0), the body-diagonal (1,1,1), a skew (2,−1,1) — to alignment **1.000** with a direction-independent magnitude (transverse fraction < 0.001). Along a fixed axis it is a sign-locked, quantised ladder in the winding: L_z = −45703/−22992/0/+22992/+45703 across q = −2…+2 (exact ±1×/±2×). Rotate the line and its spin vector rotates with it — spin has a direction.
- **V2 ✓ The winding is a dynamically-conserved charge, and the spin axis is stable.** Seeded once and co-evolved under the κ-detuned CGL with **no re-imprinting**, the line keeps its integer winding (drift 0.000 over the run) and its axis (alignment ≥ 1.00), robust to a 15% noisy seed. Because a line ∥ an axis wraps a torus cycle, its winding needs no compensating defect and its L is *more* stable than the 2D point vortex (which drained).
- **V3 ✓ Matter pins the line; like survive, unlike annihilate.** Threaded through a mass pair's κ wells, two like-winding lines both keep |w| = 1 with their cores held (min|ψ| = 0.01), while a line and an antiline **annihilate** — both windings unwind to 0 and the amplitude heals (min|ψ| → 0.66). The 3D echo of the 2D vortex selection rule.

Reproduce with `python experiments/n3_vortex_3d.py` (≈ 3 minutes; `--quick` for a smoke run); 3D checks (L aligns with the line at any orientation, sign-locked & quantised, winding reads the integer, the self-sustained line conserves winding & axis, line–antiline annihilate) in `tests/test_vortex_3d.py`.

**Honest scope:** the 3D defect is a vortex line and its angular momentum is a genuine axial **vector** — spin has a direction (V1), conserved with its axis under the self-sustained CGL (V2), matter-pinned with a real sign rule (V3). But this U(1) field carries an **integer / axial-vector** (spin-1-like, **bosonic**) angular momentum; a genuine **half-integer spinor** — the SU(2) double cover, 4π periodicity, the fermion the quark actually is — is a *different* field structure and remains the open frontier toward fermionic matter, not what a complex order parameter gives. Field-level only: the driven 3D molecule (below) is the intermediate rung. One lattice; a torus.

### The 3D molecule: a real vector spin-torque it cannot turn — 2/3 (an honest negative)

Binding two vortex lines into a molecule (3D κ-gravity + the derived exclusion floor, each mass threaded by a line) asks the next question: can the bound object *spin*, now that its spin has a direction? `experiments/n3_molecule_3d.py`. The answer is the informative one — and it earns a **2/3**, with the spin as the honest miss, exactly as the original `n3_kappa_molecule` booked its own no-spin result.

- **M1 ✓ The spin-torque is real and vectorial.** For a floor-bound pair (derived 3D floor s⋆ = 7), the phase-current force is **tangential** to the bond (F_t = 0.025 vs F_r ≈ 0), a net torque τ = 0.175 whose sign follows the winding and whose **axis is the line's direction**: lines ∥ ẑ give τ ∥ ẑ, lines ∥ x̂ give τ ∥ x̂ (alignment 1.00 in each case); the vortex–antivortex control is null. The spin's torque is a vector — the very mechanism that spun the 2D molecule.
- **M2 ✗ But the bound molecule cannot sustain rotation** (the honest negative, scored against the 2D driven bar |Ω| ≥ 0.004). Released with the drive maintained (λ > 0), the pair twists only to a static few-degree angle and stops: |Ω_axis| ≈ 0 for *both* windings — **overdamped**. This is the κ-molecule's Q < ½ failure returning: the κ-drag that binds the pair dissipates the rotation faster than the drive builds it, and now that the medium is 3D (the line threads its whole column) it wins. The 2D vortex drive beat that drag; here it doesn't.
- **M3 ✓ The object stays bound and holds its spin.** Through the run the pair sits on its floor (sep ≈ 6.7 < 1.6·s⋆) and the field keeps its winding (|w| = 1 at each mass) and its L vector (L_z ≈ ±24800) — so the failure is specifically the *rotation* (the medium's drag), not the binding or the spin content. The 3D form *carries* a spin vector; it simply cannot turn it.

Reproduce with `python experiments/n3_molecule_3d.py` (≈ 15 minutes; `--quick` for a smoke run); molecule checks (the torque axis follows the line and sign-locks, the antivortex torque is null, the driven bound pair is overdamped — no spin — yet stays bound and holds its winding) in `tests/test_vortex_3d.py`.

**Honest scope:** the κ-drag that overdamps the pair *is* the same viscosity that binds it — the κ-molecule's Q < ½, now in 3D (not a coding limit); the 2D vortex drive overcame it because the 2D medium is thinner. The driven field here is the **pinned** mode (lines re-imprinted so the torque is continuously present — the best case for spinning), and it still won't turn; the self-sustained mode additionally loses its winding at the tight 3D floor (the cores overlap). What *does* work — spin as a conserved axial vector — is `n3_vortex_3d` (field-level, 3/3). A 3D molecule that genuinely turns needs a thinner medium (higher Q) or a different binding — the open rung. One operating point (the derived 3D floor), equal masses, a 36³ torus.

### Half-integer spin: the nematic ½-disclination and its 4π double cover — 3/3

Every spin above was **integer** — a U(1) complex order parameter, a boson. A quark is a **half-integer spinor**: a 2π rotation gives −1, only 4π returns it (the SU(2) double cover of SO(3)). That needs a *different* order parameter — a **nematic**, a headless director n̂ ≡ −n̂ (order-parameter space RP¹), whose defects are **±½ disclinations** (the director winds by only π, impossible for a vector). A 2D nematic is carried by ψ = e^(2iθ) (the *doubled* angle), so the director is θ = ½·arg(ψ) and a ψ-vortex of charge q is a disclination of strength s = q/2 — the whole vortex machinery applies, but the observable director is double-valued in the lift, and *that* is the spinor. `experiments/n3_spinor.py` / `project_genesis/nematic_spinor.py`. **3/3 registered predictions land:**

- **S1 ✓ The elementary defect is a half-integer disclination.** Across the charge ladder, s(q) = q/2 exactly: s = −1.0/−0.5/0/+0.5/+1.0/+1.5 for q = −2…+3. The elementary q = ±1 defect is a **±½ disclination** — the director winds by ±π — half-integer, impossible for a vector (integer-winding) field. Odd q → half-integer, even q → integer.
- **S2 ✓ The double cover — the spinor's −1.** Transporting the *oriented* director once (2π) around a +½ disclination **flips** it: n̂·n̂ = −1.00 after 2π, and +1.00 after 4π (restored). An *integer* disclination (s = 1, from q = 2) shows no flip (+1.00 already at 2π). This is the SU(2) double cover of SO(3) — the defining property of half-integer spin — measured directly.
- **S3 ✓ Conserved, fused, bound to matter.** Pinned to a κ well and co-evolved under the CGL with **no re-imprinting**, a ½ disclination keeps its strength (s = +0.50) *and* its −1 holonomy (a conserved topological charge on matter). Two ½ disclinations **fuse** to a net integer (s = 1.00 around a loop enclosing both); a ½ and a −½ **annihilate** (s → 0, the field heals 0.00 → 0.21). The half-integer is additive and protected.

Reproduce with `python experiments/n3_spinor.py` (≈ 2 minutes; `--quick` for a smoke run); spinor checks (s = q/2, the elementary defect is half-integer, the 2π/4π double-cover holonomy, ½+½ fusion, a pinned ½ conserved with its −1 holonomy, ½/−½ annihilation) in `tests/test_spinor.py`.

**Honest scope:** this is the **order-parameter (topological)** realisation of half-integer spin — a genuine Z₂/RP¹ double cover with π-winding disclinations, the topological essence of a spinor. It is **not** the full quantum Dirac field: no spin–statistics (anticommutation), no Dirac equation, no dynamical fermion — those are the deeper frontier toward genuinely fermionic matter. What *is* measured is the half-integer winding (s = q/2), the 4π double cover, and the disclination fusion rules — and matter can now carry it. The program already has a *derived* exclusion floor (the no-cloning binding) and now a *topological* half-integer spin; tying the two into genuine Fermi statistics is the open rung. 2D; one lattice; the elementary disclination ±½.

### Hadron-like composites: the pieces together — 3/3

The synthesis rung. The program now has a **derived binding floor** (the no-cloning exclusion), **κ-gravity** to hold composites, a **driven chiral field** that spins a bound pair, and a **half-integer topological spin** (the ±½ disclination). `experiments/n3_hadron_spin.py` combines all four and asks what composite objects they make. The answer is the meson/baryon statistics split, from nothing but additivity. **3/3 registered predictions land:**

- **B1 ✓ The composite spin spectrum is additive, and its statistics alternate.** Around n = 1/2/3/4 bound constituents (κ wells at the derived floor spacing, a +½ disclination each, the field co-evolved with no re-imprinting), the far-field strength is exactly s = n/2 (+0.50/+1.00/+1.50/+2.00), and the far-field 2π holonomy alternates −1/+1/−1/+1: **odd counts are fermionic composites (quark-like, baryon-like), even counts bosonic (meson-like)** — hadron statistics from counting, exactly as for real hadrons (mesons = 2 quarks = bosons; baryons = 3 quarks = fermions).
- **B2 ✓ Half-integer inside, composite outside.** At the seed every constituent reads its own local s = ½ with the −1 holonomy; after co-evolution the total charge stays **confined and conserved** — the radial profile jumps from 0 (inside the constituent shell) to n/2 across it, and is constant at n/2 on every loop enclosing the shell (R ≥ 15). The half-integer content lives inside the composite; outside, only the total spin is visible. (For n = 2 the constituents stay pinned on their wells; for n = 3, 4 they settle into a tight ring just outside them — the n = 4 shell reaches R ≈ 10 — the total charge still confined: the *composite* is the stable object.)
- **B3 ✓ The meson is a real, spinning molecule.** Assembled dynamically — κ-gravity + the derived exclusion floor binding the pair, the self-sustained driven field (seeded once, λ = 0.2, no re-imprinting) spinning it — the 2-constituent composite stays bound on its floor (sep 8.3), keeps both constituent windings (min|w| = 1.0: the local ½'s alive), holds a persistent spin (Ω = +0.019), and the far field reads **integer s = +1.00 with holonomy +1.00**: a bound, spinning, integer-spin object made of two half-integer constituents. A boson built from two topological fermions.

Reproduce with `python experiments/n3_hadron_spin.py` (≈ 8 minutes; `--quick` for a smoke run); composite checks (s = n/2 additivity with the odd/even holonomy alternation) in `tests/test_spinor.py`.

**Honest scope:** the statistics are **topological** (the far-field double-cover class of the composite), not quantum spin–statistics — no anticommutation, no Pauli principle between identical composites; that frontier stands. The constituent count n is put in by hand (n wells) — no dynamical confinement mechanism selects 2 or 3 (the N⋆ = 3 sector story is the standing suggestion, taken up next). n = 3, 4 are static-well + evolving-field runs (the molecule dynamics is 2-body). 2D, one lattice, one operating point (the derived floor spacing), all-+½ constituents (the ½/−½ channel annihilates, per `n3_spinor`).

### The junction is the fermion: N⋆ = 3 selects the constituent count — 3/3

The hadron experiment's named gap — nothing *selected* the count — closes with an exact mechanism: **a spin defect's 2π phase winding must pass through every phase sector once**, so with three sectors the elementary spin-½ defect *is* a trivalent, three-sector junction — the colour-singlet triple. The count 3 is not a modelling choice; it is the sector count N⋆. Everything here is **emergent**: fields grown from noise (defects formed by the CGL dynamics, never imprinted), the tessellation read off the field, defects found by plaquette winding. `experiments/n3_junction_fermion.py`. **3/3 registered predictions land:**

- **J1 ✓ The spin defect and the three-sector junction are the same object.** Across seeds (36 emergent defects, net winding 0 — ± pairs), every defect sits on a junction where all 3 phase sectors meet (within 0.7 sites) and every junction site sits on a defect (within 0.7); all three sectors appear on every resolvable defect's loop, and every isolated defect carries the **singlet 1:1:1** sector composition. One object, in ± pairs. (Four defects were caught in tight ± dipoles — annihilation in flight, the ½/−½ meson channel of `n3_spinor` happening on its own — their loops reading the pair's sum, as they must.)
- **J2 ✓ That junction is the fermion.** Read as a nematic, every resolvable emergent defect carries **s = q/2 = ±0.50** with the −1/+1 (2π/4π) double-cover holonomy — while two-sector **walls** carry s = 0.00 and no flip. The elementary half-integer object and the three-sector singlet are one and the same: the baryon's "3" *is* N⋆.
- **J3 ✓ Only at N⋆ = 3 do matter and spin agree.** The defect forces all P phase sectors to meet — measured sector-valence 3.00/4.00/5.00 for P = 3/4/5 — while the *matter* tessellation (multiphase Allen–Cahn) caps its generic junctions at the Plateau valence d+1 = 3 regardless: 3.00/3.01/3.02. The two structures coincide **only at P = 3**: the three-sector world is the unique one whose own junctions can carry the elementary spin-½.

Reproduce with `python experiments/n3_junction_fermion.py` (≈ 3 minutes; `--quick` for a smoke run); junction-fermion checks (plaquette winding finds imprinted pairs, emergent defects are three-sector junctions carrying ±½) in `tests/test_spinor.py`.

**Honest scope:** this ties the *counts* together — the elementary fermionic defect is intrinsically three-sector, and 3 is the unique sector number where the matter tessellation's own generic junctions have that structure — a fundamental (N⋆) bleeding through into a higher manifestation (the constituent count). But the phase binning inherits N⋆ = 3 from the program's sector results rather than deriving it afresh here, and the hadron composites are still assembled at wells, not condensed out of the tessellation — hadrons *crystallising* at the junctions of a living tessellation is the open rung. Loop-based single-defect reads require resolvability (nearest neighbour beyond two loop radii); tighter ± dipoles are annihilating pairs and are reported as such. 2D, emergent fields, several seeds.

### The exchange sign: two identical ½-spinors are antisymmetric — spin–statistics measured — 3/3

The spinor result gave the **spin** (a ½-disclination flips −1 under 2π rotation) and the hadron result the **fusion** (½+½=1) — and the hadron result named the piece it left open, in its own words: *"no anticommutation, no Pauli principle between identical composites; that frontier stands."* That is the **statistics** half of spin–statistics, and it is now measured. The spin–statistics *connection* is the theorem that exchanging two identical spin-`s` objects multiplies the configuration by `(−1)^{2s}` — the *same* sign the object's own 2π rotation gives; for classical order-parameter defects this is the **Finkelstein–Rubinstein** construction (exchange is homotopic to a 2π rotation of the pair's frame). `project_genesis/spin_statistics.py` + `experiments/n3_exchange_statistics.py` measure it by **braiding**: two ψ-vortices of charge `q` (disclination strength `s = q/2`) are rotated rigidly about their midpoint; a half-turn swaps them (one exchange), and the oriented director `θ = ½ arg ψ` at the braid centre winds by `q·π`, so the exchange sign is `(−1)^q = (−1)^{2s}`. **3/3 registered predictions land:**

- **E1 ✓ The exchange sign is `(−1)^{2s}`.** One exchange of an identical pair reads **−1 for `q = 1`** (a ½-disclination — *fermionic*, antisymmetric), **+1 for `q = 2`** (`s = 1` — bosonic), **−1 for `q = 3`** (`s = 3/2` — fermionic), and the sign is independent of which way the pair is braided. Two identical topological ½-spinors pick up the −1 antisymmetry; the integer defect is a boson.
- **E2 ✓ Exchange = rotation — the connection itself.** That exchange sign *equals* the single-defect 2π **self-rotation** director holonomy for every charge (−1, +1, −1 for q = 1, 2, 3) — spin and statistics carrying the *same* `(−1)^{2s}` on one field. The connection is measured, not assumed.
- **E3 ✓ It is the exchange that carries the sign.** A **double** braid (two exchanges) reads +1 for all (`[(−1)^{2s}]² = +1`); the **far field** of a real exchange winds 0 (it sees only the total charge — that is *fusion*, ½+½=1, not exchange); a **no-swap** loop winds 0 (motion is not exchange); and a non-identical **(½, −½)** pair reads +1 (the windings cancel — the −1 needs two *identical* ½'s). Only a single exchange of identicals gives −1.

**What this establishes:** the exchange half of the spin–statistics frontier is measured at the level the programme realises spin — a genuine −1 antisymmetry under exchange of two identical topological ½-spinors, equal to their rotation sign. The programme now carries all three faces: the spin (the ±½ double cover), the fusion (½+½=1), and the statistics (exchange = `(−1)^{2s}`). Reproduce with `python experiments/n3_exchange_statistics.py` (≈ 2 minutes; `--quick` for a smoke run); checks (exchange sign per charge, arc-direction independence, double-braid/far/no-swap/mixed controls, exchange == self-rotation) in `tests/test_spin_statistics.py`.

**Honest scope:** this is the **topological / geometric** exchange sign of classical order-parameter defects (Finkelstein–Rubinstein) — the exact level at which this programme realises the spin, and the true content of the spin–statistics *connection* for these objects — but it is **not** quantum-field anticommutation: no Fock space, no `{ψ, ψ†}`, no many-body Pauli principle between identical *excitations*. The braid is kinematic (imprint centres moved through configuration space); a dynamical braid (κ-pinned cores co-evolved under the CGL) is the natural strengthening and should agree, the winding being a topological invariant of the path. 2D, one lattice, elementary disclination ±½.

### The dynamical braid: the exchange sign under genuine dynamics, and its limit — 3/3

The exchange sign above is *kinematic* — the field re-imprinted at every braid step, the winding imposed. The dynamical question, which its scope named, is whether a defect that **co-evolves** under the κ-detuned CGL (pinned to a κ well but never re-imprinted) keeps its winding as the well moves, so the braid and its −1 are realised by the dynamics. `project_genesis/spin_statistics.py` (`gaussian_wells`, `transport_defect`, `dynamical_braid`) + `experiments/n3_dynamical_braid.py` run it. The finding has a clean mechanism: a κ well is an **amplitude** hole; it does not constrain the phase winding, so when it moves the amplitude hole follows instantly but the winding must migrate diffusively — transport is **adiabatic with a speed limit**. **3/3 registered predictions land:**

- **D1 ✓ Transport is adiabatic, with a threshold.** A single ½ pinned to a *moving* κ well keeps its winding at low speed (`|w| = 1` at `v ≈ 0.03`) and **loses** it above a critical speed (`|w| → 0` by `v ≈ 0.1`) — a genuine adiabatic speed limit, absent from the imprinted braid where the winding is imposed. Move the well faster than the winding can migrate and the vacated core heals before it follows.
- **D2 ✓ Static co-evolved pinning is robust.** A ½ pinned to a *static* well, co-evolved 2000 steps with no re-imprint, keeps its winding (`−1 → −1`) *and* its `−1` double-cover holonomy (`−1 → −1`) — the topological charge and its spinor sign are dynamically conserved.
- **D3 ✓ (qualified) The fermionic exchange is dynamically realised — partway.** Braiding two co-evolved ½'s, the braid-centre winding carries the fermionic sign (`−0.77`, toward −1) while both cores keep their winding through **92%** of the braid — the −1 emerges from co-evolution, not imprinting. **Honest boundary:** *full* clean two-body recovery is bounded by amplitude-only pinning — the cores unwind near completion because a moving κ well transports position, not winding.

**What this establishes:** the kinematic exchange sign is dynamically meaningful — a κ-pinned ½ is transported with its winding conserved (adiabatically, threshold measured) and its −1 holonomy intact, and a two-body braid realises the fermionic sign from co-evolution partway. The exact limit is mechanistic: amplitude-only pinning cannot robustly carry a two-body winding braid to completion — the **same amplitude-vs-phase gap** that separates a topological-defect spinor from a true dynamical fermion field, so the named cure (a term that *anchors the winding*, not just the amplitude) is also the bridge toward the Fock-space anticommutator. Reproduce with `python experiments/n3_dynamical_braid.py` (≈ 3 minutes; `--quick` is a smoke run — D3 is genuinely at the edge, so its qualified pass wants the default resolution); checks (well shape, static survival of winding + holonomy, the transport threshold slow-keeps/fast-loses, the braid run) in `tests/test_dynamical_braid.py`.

**Honest scope:** 2D, relaxational (λ=0) κ-detuned CGL, Gaussian κ wells (amplitude pinning only); the transport threshold and braid operating point are one lattice and one well shape (deep, moderately tight — shallow/wide wells pin too weakly to transport at all); the centre-winding is a phase integral on a co-evolving field and carries dynamical noise (hence the qualified D3, and the sensitivity of the two-body braid to the operating point). Phase-anchoring pinning and larger lattices are the registered next rung.

### Phase-aware pinning: closing the dynamical-braid boundary — 3/3

The dynamical braid's D3 boundary was diagnosed precisely: the κ well pins **amplitude**, not phase winding, so a two-body braid unwinds near completion. `n3_phase_pinned_braid.py` adds the named cure — a **phase-aware pin**: a weak, local restoring force (rate `η`) toward the winding template `|ψ|·e^{iθ_target}` at the moving wells (a gauge-like coupling that carries topological charge), the field still co-evolving under the CGL between applications (a finite-rate force, *not* a per-step re-imprint). It closes the boundary, and — the point that makes it honest rather than circular — it does so without fabricating the answer. **3/3:**

- **P1 ✓ The cure works.** Amplitude-only (`η = 0`) reaches sign `−0.57` at 93% survival — the D3 boundary, reproduced. With the phase pin the braid **completes**: sign `−0.99` at **100%** core survival. The fermionic −1 now emerges cleanly from co-evolution, no re-imprint.
- **P2 ✓ A weak pin, with a threshold.** Sweeping `η`, the sign sharpens `−0.57 → −0.95 → −0.98 → −0.99` and survival rises to 100% past a *modest* `η⋆ ≈ 0.2` — a finite-rate force far from the hard reset of full re-imprinting.
- **P3 ✓ The pin transports winding; it does not fabricate the sign** (the decisive control). Under the *same* working pin, an **integer** (`s = 1`) pair braids to **+1** (a boson) and a **double** `½` braid to **+1** (two exchanges). So the `−1` of the single ½-exchange comes from the braid **geometry** — a single exchange of identicals — exactly as in the kinematic result; the pin only carries whatever winding is present to completion.

**What this establishes:** the D3 boundary was **amplitude-only pinning, not the physics of the exchange sign.** Supply the ingredient the amplitude well lacks — a phase-anchoring (gauge-like) coupling — and two identical ½-spinors braid to a clean, dynamically-realised −1, with the integer and double-braid controls confirming the sign is the braid's, not the pin's. The exchange antisymmetry is now carried by co-evolution rather than imprinting. Reproduce with `python experiments/n3_phase_pinned_braid.py` (≈ 4 minutes; `--quick` for a 2-point smoke run); checks (the mask, pin improves survival + sign, integer/double stay bosonic under the pin, `phase_pin = 0` is bit-compatible with amplitude-only) in `tests/test_phase_pinned_braid.py`.

**Honest scope:** the phase pin is a classical, local restoring force toward a winding template (a *background* gauge-like coupling), not a dynamical gauge field solved self-consistently, and this remains a classical order-parameter field — **not** Fock-space anticommutation (no `{ψ, ψ†}`, no many-body Pauli principle between identical excitations). What it closes is the *transport* boundary — the dynamical braid completes and the fermionic sign is genuine (geometry, not fiat). A self-consistent gauge field and the quantum fermion are the standing frontier.

### The self-consistent gauge field: the vortex becomes a gauged particle — 3/3

The phase-pinned braid used a *background phase template* to anchor the winding, and named its caveat: not a dynamical gauge field solved self-consistently. This rung supplies the real thing. `project_genesis/gauged_vortex.py` couples the vortex to a genuine **U(1) gauge field** — the textbook **abelian Higgs / Ginzburg–Landau** model (a lattice superconductor), with the scalar ψ on sites, U(1) link phases `θ_μ` as the connection, and the gauged energy `E = Σ_μ |ψ − U_μ ψ(+μ)|² + (β/2)ΣB² + (λ/4)Σ(|ψ|²−1)²` — and gradient-flows *both* fields to the self-consistent vortex (the Abrikosov / Nielsen–Olesen flux tube). `experiments/n3_gauge_field.py` measures the three things a real gauge field gives that the template did not. **3/3:**

- **G1 ✓ Flux quantization.** A winding-`q` vortex forces the self-consistent gauge field to carry a **quantized** magnetic flux `Φ = 2π·q` through a loop about its core — `q = 1, 2, 3 → −1.00, −1.99, −2.99` flux quanta, *solved by the dynamics, not imposed* — while the *global* vortex (gauge field frozen off) carries **zero** flux. The gauge field is the real anchor of the winding.
- **G2 ✓ Finite energy — London screening.** With the gauge field on, the vortex–antivortex interaction energy **saturates** with separation (`≈ 23.8` flat, a finite-range, screened force → finite-energy solitons); with it off, the global vortices' energy keeps **growing** (`1 → 9 → 79 → 97` — the logarithmic long-range tail). The gauge field screens the divergence: the vortex is a finite-energy soliton.
- **G3 ✓ Gauge invariance.** Under a random *local* gauge transformation `ψ → e^{iα(x)}ψ`, `θ_μ → θ_μ + α(x) − α(x+μ)`, the observables (energy, flux) are invariant to **machine precision** (`ΔE/E = 0`, `ΔΦ ≈ 4×10⁻¹⁵`) — the signature of a genuine gauge theory, which the fixed phase template of the pin was not.

**What this establishes:** the vortex is now a genuine **gauged particle** — its winding anchored by a self-consistent, quantized, gauge-invariant magnetic flux, of finite energy. The phase-template caveat is closed with the real field: the "background gauge-like coupling" is replaced by a solved U(1) gauge theory, and the winding it anchors is a physical flux quantum, not a reference phase. This is the field-theoretic ground the exchange statistic must ultimately stand on. Reproduce with `python experiments/n3_gauge_field.py` (≈ 2 minutes; `--quick` for a smaller scan); checks (seeding + flux, energy descent, flux quantization at 1/2 quanta, gauge-off carries no flux, gauge invariance of energy + flux) in `tests/test_gauged_vortex.py`.

**Honest scope:** this is the **classical** abelian Higgs gauge field — a genuine, self-consistent, gauge-invariant U(1) theory, but a classical one: **no** Fock space, **no** `{ψ, ψ†}` anticommutator, **no** many-body Pauli principle between identical excitations. The **Aharonov–Bohm / Chern–Simons statistical phase** — the flux attachment by which this gauge field would turn the gauged ½-vortex's topological exchange sign into a genuine dynamical *fermion* statistic — is the standing next rung, and the true bridge to a quantum fermion. 2D, one lattice, gradient-flow (relaxational) dynamics, one (λ, β) operating point.

### The Aharonov–Bohm statistical phase: the fermion's −1 as a gauge holonomy — 3/3

The self-consistent gauge field sets up the deepest step in the fermion arc: showing that the exchange sign — the `−1` — is not only a *topological* invariant (the braid) and a *geometric* one (the 2π self-rotation) but a **dynamical gauge phase**, the **Aharonov–Bohm** phase of the flux the gauge field carries. This is **Wilczek's statistical transmutation**: a composite of charge and flux exchanges with a phase set by the flux it drags, and one flux quantum on a unit charge turns a boson into a **fermion**. `experiments/n3_ab_statistics.py` measures it on the programme's own relaxed gauge field (`gauged_vortex.wilson_loop` / `ab_phase`). **3/3:**

- **AB1 ✓ The Aharonov–Bohm phase is the enclosed flux.** The Wilson-loop holonomy of the self-consistent gauge field about a winding-`q` vortex is `Φ = 2π·q` (measured `−1.00, −1.99, −2.99` quanta for q=1,2,3); a **unit** test charge encircling it gets `e^{iΦ} = +1` (Dirac — an integer flux quantum is invisible to an integer charge), while a **half** charge gets `e^{iΦ/2} = (−1)^q` — the flux quantum is visible to a fractional charge as a sign.
- **AB2 ✓ Statistical transmutation — the flux–charge composite is a fermion.** Exchanging two identical composites (charge 1, flux Φ) is *half a braid*, so each charge sees half the partner's flux: the statistical phase is `θ = Φ/2 = πq` → exchange sign `(−1)^q`. **One flux quantum (q=1) makes a fermion (−1); two (q=2) a boson (+1).** The gauge field turns the topological winding into a dynamical exchange statistic.
- **AB3 ✓ The three faces of `(−1)^{2s}` agree.** Per q, the Aharonov–Bohm gauge phase, the *topological* braid exchange sign (`spin_statistics.exchange_holonomy`), and the *geometric* 2π self-rotation holonomy (`self_rotation_sign`) read one and the same value (`q=1: −1/−1/−1`, `q=2: +1/+1/+1`, `q=3: −1/−1/−1`) — **spin, statistics, and gauge, one sign.**

**What this establishes:** the fermion's exchange sign is now realised on the programme's own self-consistent gauge field as an **Aharonov–Bohm holonomy** — the field-theoretic mechanism by which a gauge field produces fermionic statistics from a bosonic field (flux attachment). The `−1` that began as a topological winding is now a *dynamical gauge phase*, and spin (the 2π rotation), statistics (the braid), and the gauge holonomy are three measured faces of the one `(−1)^{2s}`. Reproduce with `python experiments/n3_ab_statistics.py` (≈ 2 minutes; `--quick` for a smaller scan); checks (the Wilson-loop holonomy = enclosed flux, the AB phase of a unit vs half charge, the statistical sign) in `tests/test_ab_statistics.py`.

**Honest scope:** this is the **classical** abelian statistical phase — the Wilczek flux–charge composite at the level of a classical gauge field and its Aharonov–Bohm holonomy. It is **not** Fock-space anticommutation: no `{ψ, ψ†}` operator algebra, no many-body Pauli principle between identical *quanta*, no Chern–Simons action solved dynamically (the flux–charge binding is read off, not enforced by a CS term). **Second quantization** — the operator algebra and the dynamically-solved statistics — remains the frontier, and it is the one that genuinely exceeds a classical PDE testbench.

### The gap has a scaling dimension: distinction volume-law, integration area-law — 2/3 (a refuted hypothesis)

The URP corpus asserts **holography as the geometric consequence of the distinction–integration gap**: a field makes more distinctions than it can locally integrate, so integration concentrates on boundaries. That claim had never been measured anywhere in this repo. `project_genesis/area_law.py` + `experiments/n3_area_law.py` measure it — and refute the mechanism the experiment itself pre-registered.

For nested regions of half-width `ℓ` in the 2-D sector field: distinction content `C(ℓ) = Σ|∇Ψ|²`, integration content `I(ℓ)` = region↔complement coherence (the Gaussian mutual-information proxy `ρ²`). Exponent 2 is a volume law, 1 an area law.

- **A1 ✓ Calibrate before believing anything.** On a Gaussian control whose mutual information obeys an *exact* area law, a **single realisation reads `1.77` against a truth of `1.0`** — the correlation noise floor convolves into a spurious volume term. Ensemble averaging plus floor subtraction brings the estimator to `1.10–1.13`, a measured bias of **`+0.12`** quoted with every number below. Without this step the whole experiment is unsafe — the first run of it produced a confident, wrong answer.
- **A2 ✓ The asymmetry, and it is exactly one dimension.** Distinction is volume-law (`n_C = 1.97–2.01`) while integration is area-law (`n_I = 0.94–0.98` bias-corrected): **`n_C − n_I = 1.03`**. The field distinguishes over the volume and can only integrate over the surface. *The generative gap is not just a magnitude — it is a whole dimension of the region.*
- **A3 ✗ REFUTED — and the truth is stronger.** The registered hypothesis, in this programme's own grammar (*X emerges when capacity binds*), was that abundant κ gives volume-law integration and scarcity forces it onto the boundary. **There is no crossover.** `n_I` moves by **0.04** across `⟨κ⟩ = 0.945 → 0.075`, while the coherence length collapses `ξ = 12 → 2`. Capacity sets the *coefficient* and the crossover scale, never the exponent.

**What this establishes:** the D–I separation is **capacity-invariant** — it cannot be bought. That is the *same signature* the capacity-separation cliff shows (`φ = I/C` pinned under a 10× κ sweep), reached from a completely independent measurement, and it is what the ordinal theorem says: `I(F) < C(F)` is structural, not a resource shortfall. The hypothesis that scarcity *forces* the area law is wrong; the field never had the option. Reproduce with `python experiments/n3_area_law.py` (≈ 7 minutes; `--quick` for a smaller scan); instrument checks (power-law recovery, the noise-floor trap, ensemble convergence, the known-area-law control) in `tests/test_area_law.py`.

**Honest scope:** for **any** classical field with finite correlation length, area-law mutual information is *expected* — a standard result, not a discovery. What is non-trivial is the **exact one-dimension gap** and its **capacity-invariance**. This does **not** derive holography, and it does **not** touch gravity: it measures coherence, not the κ well. Whether the bulk κ field is *reconstructible from its boundary* is the sharper question this sets up and does not answer. 2-D, one lattice, one coarsening time. The deepest-scarcity point (`ξ → 1`) is excluded from the verdict: the field is fragmented to the lattice spacing and the measurement breaks down — **the instrument fails exactly at the pixelation scale.**

### The condensation boundary: why matter cannot yet gather its fermions — 3/3 (headline a negative)

The last "assembled by hand" gap is **condensation**: can matter *collect* its spin-½ defects out of a noise-grown field rather than have them imprinted? The honest answer, mapped by `experiments/n3_condensation_boundary.py`, is **not yet** — and the three results say exactly why and what's missing. **3/3 registered predictions land — but the headline is the negative:**

- **K1 ✓ Passive traps do not condense — the dilute gas is frozen.** With ψ grown from noise over κ wells at floor spacing, the well occupancy stays at chance (0/12 vs 0/12 across trap depths γ = 0.8/0.95 and coarsening times), and pinning gives no survival enhancement (defect counts match the flat-κ control). A defect born outside a well's few-site basin never reaches it — there is no transport in the relaxational gas.
- **K2 ✓ The naive third-law reaction force clumps.** The detuning energy (wells suppress |ψ|²) implies a force pulling a mass toward amplitude holes — but *every mass's own well is an amplitude hole*, so the raw force −χ⟨∇|ψ|²⟩ collapses a floor-bound pair through its exclusion floor (late sep 1.5–2.5 vs the phase-only control's 8.5–8.6, s⋆ = 8). A force that cannot tell matter from fermion condenses nothing.
- **K3 ✓ The reaction force *can* be made selective — the working ingredient.** Divide the wells' reversible envelope out (react to |ψ|²/(1−g)) and it responds only to the *topological* core: normalized pull toward a fermion core +2.4×10⁻² vs an empty well **+8.8×10⁻¹⁸** (~0), where the raw force is fooled by both (+5.5×10⁻² vs +4.4×10⁻²). A force that can tell a fermion from a fellow mass.

Reproduce with `python experiments/n3_condensation_boundary.py` (`--quick` for a smoke run); condensation checks (the raw force points at a core, the normalized force ignores an empty well but still sees a core, the gas comes in balanced ± pairs) in `tests/test_condensation.py`.

**Honest scope — the headline:** full condensation is **not** achieved. Matter cannot yet *gather* fermions from afar to build a composite: the dilute gas is frozen (K1, no transport), the naive force clumps matter instead (K2), and while the envelope-normalized force is the right selective handle on a fermion (K3), a selective force is useless without a **current** to carry fermions to it. (The exploratory mobile-channel probe confirmed it directly: masses released into the gas show no systematic capture at working density.) The missing ingredient is defect transport, and the named candidate is the same λ ≠ 0 precession that regenerated the driven spin — spiral-wave advection mobilizes vortex cores — the honest next rung, at which the whole chain (sector → fermion → composite) would finally run itself. The normalized reaction force is a *motivated* selectivity (its coefficient free, not derived); 2D, one lattice.

### The transport current: matter gathers its fermions — 3/3 (the boundary lifted)

The condensation boundary named its own missing piece exactly: a **transport current** to carry spin-½ defects to matter. The named candidate was the same **λ ≠ 0 precession** that regenerated the driven spin — the CGL twist turns each vortex core into a drifting spiral source. `experiments/n3_condensation_transport.py` adds that term and asks whether it closes the boundary. It does — **partly, and honestly. 3/3 registered predictions land, and the boundary's headline negative is lifted:**

- **L1 ✓ The λ precession is a transport current.** A relaxational (λ = 0) detuned CGL settles to rest — its late-time field velocity ⟨|ψ_{t+1} − ψ_t|⟩ decays to ~0 (**0.0002**) — while a λ > 0 field sustains a persistent spiral current whose magnitude grows monotonically with λ (**0.047** at λ = 0.5, **0.100** at λ = 1). This is the direct signature of the transport the frozen relaxational gas lacked. (Nearest-core displacement was tried first as a proxy but is seed-noisy and confounded by pair annihilation; the field velocity is the robust measure.)
- **L2 ✓ Transport enables condensation — the wells now gather fermions.** With the current on, the κ wells (amplitude minima a mobilized core falls into) trap defects from the noise-grown gas above chance: at λ = 1 occupancy reaches **4/12 wells (⅓)** against a flat-κ chance baseline of **0/12** and the frozen λ = 0 gas's 1/12. The well−chance excess grows with λ. Matter *gathers* its fermions — K1's frozen boundary, lifted.
- **L3 ✓ What condenses is a fermion.** The trapped defects are genuine ±½ disclinations — every one caught reads s = ±½ with the **−1 double-cover holonomy** — and the wells are **singly** occupied (the exclusion analog: a well holds about one). A real spin-½ has condensed onto the matter, from noise, with no imprinting.

Reproduce with `python experiments/n3_condensation_transport.py` (`--quick` for a smoke run); the transport checks (a λ = 0 field relaxes to rest while a λ > 0 field sustains a current; a grown core reads s = ±½ with the −1 holonomy) in `tests/test_condensation.py`.

**Honest scope:** this is **partial** self-assembly — a *controlled excess* of occupancy over chance (about ⅓ the wells at λ = 1), not yet wall-to-wall condensation. The transport window is bounded above (λ ≳ 1.5 is numerically unstable at dt = 0.1, and a very mobile core can annihilate before it traps), and a fraction of captures can be spurious near-well features rather than clean ½'s (seed-dependent, 0 to ~1/6 across runs). This is the *field-level* transport — the gas is mobilized and trapped by the wells' amplitude minima; combining it with the mass-side selective reaction force (the boundary's K3) into a single self-assembling molecule is the next rung.

### Self-assembly: a molecule binds and gathers a fermion — 3/3 (with two honest boundaries)

The chain is now complete except for one join: put the two *dynamical* halves together — the mass-side **selective reaction force** (the condensation boundary's K3, which lets a mass hold a topological core while ignoring a fellow mass) and the field-side **transport current** — in a single co-evolution, and ask the culminating question: does a molecule **assemble itself** from noise? Mapped by `experiments/n3_self_assembly.py`, **it does — in a window, and 3/3:**

- **S1 ✓ The molecule self-assembles at the derived floor.** Two masses released *beyond* the floor (start 11) settle to a bound separation on **6/6 seeds** — the bare κ-gravity + exclusion floor s⋆ ≈ 8.5, a little tighter (~6–7) when a core is caught between them. The parameter-free binding, reached by self-assembly rather than placement.
- **S2 ✓ The molecule gathers and holds a ±½ from the noise-grown gas.** With the transport current on, a mass draws a ±½ defect out of the gas and holds it while bound on **6/6 seeds** (λ = 0 frozen control 4/6) — the gathering of the transport experiment, now onto a mobile, self-assembled mass rather than a fixed well.
- **S3 ✓ What decorates the molecule is a genuine spinor — from noise.** The captured cores read **s = ±½ with the −1 double-cover holonomy** on 6/6 captures; the whole molecule emerged from noise — gas grown, binding derived, nothing imprinted.

Reproduce with `python experiments/n3_self_assembly.py` (`--quick` for a smoke run); the assembly checks (opt-in ψ snapshots return the field at each record; the masses bind near the derived floor from a wider start) in `tests/test_condensation.py`.

**Honest headline — two boundaries remain.** (1) *The hold is imperfect*: a mass holds its captured ±½ for a majority but not all of the run (~60%) — the defect wanders in and out of the capture radius as the molecule jostles, so the decoration is an intermittent bond, not a locked permanent state (and a full-strength λ current annihilates the gas before it can be caught, hence the moderate operating point). (2) *Transport is not cleanly isolated*: the frozen λ = 0 control still captures (4/6), because the selective force's capture-and-hold already grabs *born-near* defects without any transport — so what transport uniquely buys, gather-from-afar, is only a modest edge at this density (6/6 vs 4/6). A locked decorated ground state (or a replenished gas source) and a clean transport isolation (tracing the captured defect's origin) are the next rungs. 2D, one lattice, several seeds, the derived floor spacing.

### The gas source: the decoration becomes persistent — 2/3 (transience lifted, purity the new boundary)

Self-assembly named the intermittent hold's cause: the noise-grown gas is **sparse** and coarsens (pairs annihilate, density decays), so a trap is only occasionally within reach of a defect. The fix it named was a gas *source* — continuously replenish the gas. `experiments/n3_gas_source.py` adds one as a **Langevin bath**: a noise term η·√dt·(ξ + iξ′) injected into the κ-detuned CGL each step (`sourced_gas_step`), a finite-"temperature" field that continuously nucleates fluctuations. It works — **and, honestly, introduces the next boundary in doing so. 2/3:**

- **G1 ✓ The source maintains a steady gas.** Above a threshold in η the defect density holds at a nonzero steady state over a long window — **~30 defects** (η = 0.6) vs the un-sourced gas coarsening to **~2** — the replenishment the intermittent decoration lacked. (It is a threshold, a Kibble–Zurek-like picture: below η_c the field orders and the gas coarsens away; well above it the field boils into a dense turbulent bath.)
- **G2 ✓ The decoration becomes persistent AND stays selective.** With the source on, the κ-well occupancy over the window rises to **0.85** (from the un-sourced **0.08**) while the ambient chance at empty reference points stays at **0.03** — the well stays occupied while empty space stays empty. The self-assembly's intermittent hold, lifted to a steady, selective state.
- **G3 ✗ But what is persistently held is a clean ±½ only about half the time.** The dense replenished bath also nucleates *integer* (s = ±1) and *clustered* defects, so the occupied wells read s = ±½ with the −1 holonomy on only **~47%** of snapshots — where the sparse un-sourced gas reads **100%** pure (and the transport experiment read 80–100%). Persistence bought at the cost of purity.

Reproduce with `python experiments/n3_gas_source.py` (`--quick` for a smoke run); the source checks (η = 0 reduces exactly to the un-sourced step; the bath holds a density far above the coarsening floor) in `tests/test_condensation.py`.

**Honest headline — the source lifts the transience but degrades purity.** The sparse gas was pure but transient; the sourced gas is persistent but impure. Persistence and purity trade off across the same boiling threshold — below η_c the gas coarsens away (no supply), above it the bath boils into integer/clustered defects and, higher still, erodes the wells' selectivity (the ambient chance climbs) and degrades the field amplitude. A **"cold" source** — one that replenishes clean ½'s without boiling to integer defects — is the next rung. This is the field-level source with fixed wells (the transport setting); wiring it into the full molecule co-evolution is the follow-on. 2D, one lattice, several seeds, the derived floor spacing.

### The cold source: clean fermions, but the tradeoff is fundamental — 2/3 (headline a negative)

The gas source named its fix: a **cold source** that injects the *fundamental spin-½ quantum* — clean winding-±1 pairs (`inject_defect_pair`, each winding-1 → s = ±½) — dilutely, so the supply is pure by construction and the bulk stays sparse, instead of the hot Langevin bath's indiscriminate boil. `experiments/n3_cold_source.py` builds it and asks whether it recovers purity *without* losing the persistence the hot bath bought. The honest answer is **no — and the reason is fundamental. 2/3, headline a negative:**

- **C1 ✓ The cold source is pure.** Injecting clean winding-±1 pairs holds the occupied-well ±½ purity at **82%** — vs the hot bath's **47%** — the integer/clustered contamination removed at the source.
- **C2 ✓ The cold source is selective and local.** With the source localized to the matter region, the κ-well occupancy (**0.34**) stays above the far ambient chance (**0.10**) — the supply feeds the matter while the far bulk stays comparatively empty.
- **C3 ✗ But it is not persistent.** The clean, dilute supply *self-annihilates* before it keeps the traps fed, so the **clean-½ occupancy** (occupancy × purity — the fraction of time the well holds one genuine ±½) is **0.28**, which does *not* beat the hot bath's **0.40**. Deepening the well to raise occupancy re-introduces clustering, and the same tradeoff reappears.

Reproduce with `python experiments/n3_cold_source.py` (`--quick` for a smoke run); the cold-source check (an injected pair relaxes to a clean net-zero +1/−1 pair reading s = ±½) in `tests/test_condensation.py`.

**Honest headline — the persistence–purity tradeoff is fundamental for a memoryless source.** Neither the hot bath (persistent, impure — clean-½ occ 0.40) nor the cold source (pure, sparse — 0.28) holds a *clean single* ±½ more than ~40% of the time, and every knob — source rate, locality, well depth — trades one for the other. The missing ingredient is not a cleverer source but a **single-occupancy lock**: a well that retains exactly one ½ against annihilation and excludes a second — the no-cloning **exclusion floor**, here as a trap. The cold-source rung converges with the locked-ground-state rung the self-assembly named — they are the *same* rung. Field-level, fixed wells; 2D, one lattice, several seeds, the derived floor spacing.

### The single-occupancy lock: a free ½ cannot be locked — 1/3 (the loop closes)

The cold source reduced everything to one frontier: a well that holds *exactly one* clean ±½. Diagnosing the hot bath's impurity shows the failure is not integer defects (~8%) but **clustering** — the occupied well holds ≥2 defects ~87% of the time, a clean single ±½ only ~5%. The well is a *sink* that gathers every nearby defect. Same-sign ½'s already repel (the topological Pauli analog is there); the clustering is *opposite-sign* — a well holding a +½ attracts a −½, they meet and annihilate. So the lock is **charge polarization**: a source + sink (feed one sign into the matter region, drain its anti-particle) — a battery/electrode — removing annihilation so like-charge repulsion can exclude a second. `experiments/n3_single_occupancy.py` builds it. **It works partway, then hits the wall that closes the whole loop. 1/3:**

- **L1 ✓ Charge polarization lifts single-occupancy.** The source+sink raises the *clean-single-½* fraction (exactly one defect in the well, reading a clean ±½ with the −1 holonomy) from the hot bath's **0.07** to **0.24** — several-fold, annihilation removed, like-charge repulsion doing the excluding.
- **L2 ✗ But even a singly-occupied well reads clean only ~⅔ of the time** (**0.65**): a neighbour just outside the capture radius still contaminates the reading — the exclusion is *soft*.
- **L3 ✗ And the lock never completes** (clean-single-½ caps at **~0.24**, far below persistent single-occupancy). A **free** gas ½ carries only soft (~1/r) repulsion, not the **hard derived exclusion floor** s⋆ that would keep a second core beyond the well; imposing that floor crudely at the field level (an ordered "moat" around each core) *backfires* and destroys the gas.

Reproduce with `python experiments/n3_single_occupancy.py` (`--quick` for a smoke run); the cold-source primitive it builds on (an injected pair relaxes to a clean net-zero +1/−1 pair reading s = ±½) is checked in `tests/test_condensation.py`.

**Honest headline — matter's fermions are single-occupancy because they are *bound*.** Charge polarization is the right mechanism and works partway (~3×), but the well still admits clustering because the fermions are *free* — they carry only soft repulsion, not the hard, parameter-free exclusion floor the program **derived** (`n3_exclusion`), which only **bound** structures carry (two masses cannot approach within s⋆; a free defect can). So Pauli single-occupancy is a property of *bound* fermions carrying the no-cloning floor — the self-assembly molecule, not a free gas defect. The single-occupancy lock closes the loop back to the derived exclusion floor and the bound composite: the whole matter arc, from the exclusion floor through transport, gathering, binding, persistence and purity, resolves to a single statement — **a fermion is single-occupancy matter when it is bound and carries the floor.** Field-level, fixed wells; 2D, one lattice, several seeds, the derived floor spacing.

### The chirality cascade: the leaning sorts matter by handedness — 3/3

The chiral term λ (the CGL twist 1 + iλ) has been the handedness-giver all along — it regenerated the driven spin and drove the transport current. `experiments/n3_chirality.py` asks the cascade question you reach once matter exists: does the chiral *leaning* impose a **definite handedness** on matter, distinguish matter from antimatter, and propagate to bound structure — the root of the world's homochirality? The clean, persistent probe is the **orbital rotation** of a same-charge vortex pair (two ½'s that repel, so they can't annihilate): under λ their separation vector accumulates a rotation whose *sign is a handedness*. **3/3:**

- **X1 ✓ The chiral leaning breaks parity.** The pair accumulates a handed rotation absent at λ=0 (parity holds) and growing with |λ|: **0° / 9° / 18°** for λ = 0 / 0.6 / 1.
- **X2 ✓ CP-like, and matter/antimatter are chirally opposite.** The rotation reverses under λ→−λ (+18° ↔ **−18°**) *and* with the charge sign (+½ **+18°** vs −½ **−18°**) — so a +½ and a −½ are wound *oppositely* by the same leaning: the chiral field distinguishes matter from antimatter *by handedness*.
- **X3 ✓ It cascades to bound structure.** A pair bound in a matter well inherits the *same* λ-set handedness (0° at λ=0, reversing with sign λ) and, confined, winds many times faster (**±454°**) — the chiral choice propagates undiluted from the field to the composite. Since λ's sign is *one* fixed choice for the whole lattice, all matter of a given charge shares *one* handedness: the homochirality root.

Reproduce with `python experiments/n3_chirality.py` (`--quick` for a smoke run); the handedness assay (a same-sign pair winds + for λ>0, − for λ<0, ~0 at λ=0, and flips with charge) in `tests/test_chirality.py`.

**Honest scope — handedness, not a head-count.** The chiral leaning makes matter and antimatter chirally *distinguishable* and imposes one consistent handedness (the homochirality root), but it is **not baryogenesis**: on the closed torus the net defect count stays charge-balanced (every +½ has a −½), and the transverse *drift* (as opposed to this rotation) is charge-*blind*, so even an open-system matter *excess* needs a charge-dependent removal — the named frontier. The λ-dependence — the chiral selection itself — is the universal part. 2D, imprinted probe vortices, several orientations, one lattice.
