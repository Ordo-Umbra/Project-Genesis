...
def test_control_arm_is_bit_identical_to_the_published_path():
...
def test_control_arm_matches_in_3d_too():
...
def test_control_measure_window_matches_the_published_one():
    ...
    for key in ("distinction", "integration", "integration_guarded",
                "churn", "domain_scale"):
        assert m_a[key] == pytest.approx(m_b[key], rel=1e-12)


# ------------------------------------------------------------ the stepper

def test_stepper_without_capacity_is_the_plain_law():
...
def test_stepper_with_capacity_is_the_repo_capacity_law():
...
def test_capacity_field_starts_at_baseline():
...
def test_free_capacity_stays_pinned_at_baseline():
    ...
    assert kappa.mean() == pytest.approx(1.0, abs=1e-6)
    assert kappa.min() == pytest.approx(1.0, abs=1e-6)


def test_binding_budget_actually_depletes_capacity():
    _, kappa = evolve_palette_gated(3, 16, 40, np.random.default_rng(3),
...
def test_scarcity_is_monotone_in_the_consumption_knob():
    means = []
...
def test_capacity_never_leaves_its_bounds():
...
def test_measure_reports_realised_capacity_not_the_nominal_setting():
...
def test_control_arm_reports_unit_capacity():
    ...
    m = measure_window_gated(fields, kappa, 3, 5, rng, consumption=None,
                             noise=0.2)
    assert m["kappa_mean"] == pytest.approx(1.0)


def test_gated_measure_carries_the_texture_guard():
