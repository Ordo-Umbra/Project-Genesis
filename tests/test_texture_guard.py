...
class TestTextureGuard(unittest.TestCase):
    ...
    def test_uniform_field_has_maximal_domain_scale(self):
        labels = np.zeros((8, 8, 8), dtype=int)
        self.assertAlmostEqual(domain_scale(labels), labels.size, places=7)


    def test_lattice_scale_noise_has_domain_scale_one(self):
        rng = np.random.default_rng(0)
        labels = rng.integers(0, 4, (20, 20, 20))
        self.assertAlmostEqual(domain_scale(labels), 1.0, delta=0.05)


    def test_domain_scale_grows_with_block_size(self):
    ...
    def test_the_artifact_is_real_and_severe(self):
    ...
    def test_guard_flags_the_artifact_as_unresolved(self):
    ...
    def test_guard_lowers_but_does_not_rescue_the_artifact(self):
    ...
    def test_guard_is_a_no_op_on_genuine_domains(self):
        ...
        self.assertAlmostEqual(out["density"], out["raw"], delta=abs(out["raw"]) * (1e-6) + 1e-12)


    def test_majority_filter_preserves_a_large_domain_partition(self):
    ...
    def test_majority_filter_removes_isolated_cells(self):
        labels = np.zeros((12, 12), dtype=int)
        labels[6, 6] = 1                    # a single stray cell
        assert majority_filter(labels, 2)[6, 6] == 0


    def test_majority_filter_is_idempotent_on_clean_domains(self):
    ...
    def test_majority_filter_never_invents_a_sector(self):
    ...
    def test_filter_passes_are_cumulative(self):
    ...
    def test_measure_window_reports_the_guarded_quantities(self):
    ...
    def test_guarded_integration_is_never_above_raw_across_palettes(self):
    ...
    def test_sector_labels_round_trip_through_the_guard(self):
