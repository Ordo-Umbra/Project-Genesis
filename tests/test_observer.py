...
def test_z_matches_known_quantiles():
    ...
    assert _z(0.80) == pytest.approx(0.841621, abs=1e-4)
    assert _z(0.50) == pytest.approx(0.0, abs=1e-6)


def test_smaller_effects_need_more_trials():
    ns = [power_n(0.40, d) for d in (0.30, 0.20, 0.15, 0.10, 0.05)]
    assert ns == sorted(ns)


def test_power_scales_roughly_as_one_over_delta_squared():
...
def test_power_rejects_an_impossible_shift():
    assert power_n(0.90, 0.20) == -1


def test_power_is_high_enough_to_be_honest():
...
def test_a_planted_effect_is_found():
...
def test_direction_of_the_difference_is_attend_minus_ignore():
    ...
    assert r["p_attend"] == pytest.approx(1.0)
    assert r["p_ignore"] == pytest.approx(0.0)


def test_single_arm_is_reported_not_crashed():
...
def test_load_log_accepts_a_bare_list(tmp_path):
    p = tmp_path / "t.json"
...
def test_load_log_accepts_a_wrapped_object_and_ignores_extras(tmp_path):
    p = tmp_path / "t.json"
