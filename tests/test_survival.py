...
def test_batched_step_matches_repo_kernel_exactly():
...
def test_batched_step_matches_repo_kernel_in_2d():
...
def test_seed_batch_is_on_the_unit_sphere():
...
def test_multiplicity_of_a_uniform_field_is_one():
    labels = np.zeros((2, 5, 5, 5), dtype=int)
    m = multiplicity_batch(labels, 3, 3)
    assert (m == 1).all()


def test_multiplicity_counts_a_two_sector_interface():
...
def test_multiplicity_never_exceeds_palette():
...
def test_sector_labels_pick_the_largest_component():
    fields = np.zeros((1, 3, 2, 2, 2))
    fields[0, 2] = 5.0
    assert (sector_labels_batch(fields) == 2).all()


def test_window_features_returns_all_names_per_replica():
...
def test_run_ensemble_records_every_run():
...
def test_censored_runs_are_kept_not_dropped():
...
def test_kaplan_meier_starts_at_one_and_decreases():
...
def test_kaplan_meier_with_no_deaths_is_flat():
...
def test_split_hazard_is_unbiased_for_a_constant_rate_process():
...
def test_hazard_ratio_ci_covers_one_for_a_memoryless_process():
...
def test_split_hazard_detects_a_falling_rate():
    ...
    assert h["ratio"] == pytest.approx(0.0)


def test_hazard_exposure_only_counts_time_actually_at_risk():
    ...
    assert h["early_exposure"] == pytest.approx(600.0)


# ----------------------------------------------------------------- statistics

def test_spearman_of_a_monotone_map_is_one():
    x = np.arange(20, dtype=float)
    assert spearman_r(x, x ** 3) == pytest.approx(1.0)
    assert spearman_r(x, -x) == pytest.approx(-1.0)
...
def test_roc_auc_of_a_constant_score_is_one_half():
    assert roc_auc(np.ones(20), np.arange(20) < 10) == pytest.approx(0.5)


def test_held_out_auc_on_pure_noise_sits_near_chance():
...
def test_held_out_auc_finds_a_planted_signal():
...
def test_permutation_p_is_uninteresting_for_noise():
    ...
    assert p > 0.05
    assert null_mean == pytest.approx(0.5, abs=0.12)


def test_look_elsewhere_null_is_stricter_than_a_single_feature_null():
...
def test_max_feature_permutation_reports_the_actual_winner():
...
def test_confirm_feature_rejects_a_null_feature():
...
def test_confirm_feature_is_direction_free():
