...
class TestExpansion(unittest.TestCase):
    ...
    def test_every_schedule_starts_at_unity(self):
        for kind in SCHEDULES:
            self.assertAlmostEqual(scale_factor(kind, 0.0, 0.1), 1.0, places=7)


    def test_zero_rate_is_a_static_box_for_every_schedule(self):
        for kind in SCHEDULES:
            for t in (0.0, 10.0, 500.0):
                self.assertAlmostEqual(scale_factor(kind, t, 0.0), 1.0, places=7)


    def test_flat_never_expands_however_fast_the_rate(self):
        self.assertAlmostEqual(scale_factor("flat", 1000.0, 5.0), 1.0, places=7)


    def test_schedules_are_monotone_increasing(self):
        for kind in ("radiation", "matter", "desitter"):
            a = [scale_factor(kind, t, 0.05) for t in (0, 5, 20, 100, 400)]
            assert all(x <= y for x, y in zip(a, a[1:]))


    def test_late_time_power_law_exponents_are_right(self):
        ...
        for kind, want in (("radiation", 0.5), ("matter", 2.0 / 3.0)):
            ...
            self.assertAlmostEqual(slope, want, delta=1e-3)


    def test_desitter_grows_faster_than_any_power_law(self):
        t = 300.0
    ...
    def test_diffusion_is_one_over_a_squared(self):
        d = diffusion_schedule("matter", 0.05, 0.06)
        for step in (0, 50, 500):
            a = scale_factor("matter", step * 0.06, 0.05)
            self.assertAlmostEqual(d(step), 1.0 / (a * a), places=7)


    def test_diffusion_starts_at_one_and_only_falls(self):
        d = diffusion_schedule("desitter", 0.05, 0.06)
        vals = [d(s) for s in (0, 10, 100, 500, 1000)]
        self.assertAlmostEqual(vals[0], 1.0, places=7)
        assert all(x >= y for x, y in zip(vals, vals[1:]))


    def test_flat_schedule_leaves_diffusion_at_one(self):
        d = diffusion_schedule("flat", 0.0, 0.06)
        for s in (0, 100, 5000):
            self.assertAlmostEqual(d(s), 1.0, places=7)


    def test_floor_clamps_decoupling_when_asked(self):
        d = diffusion_schedule("desitter", 0.5, 0.06, floor=0.01)
        self.assertAlmostEqual(d(10000), 0.01, places=7)


    def test_expansion_only_touches_the_laplacian(self):
    ...
    def test_lower_diffusion_really_does_slow_the_field(self):
    ...
    def test_late_metrics_have_one_value_per_run(self):
    ...
    def test_late_metrics_are_in_range(self):
    ...
    def test_a_frozen_field_has_zero_churn(self):
        ...
        self.assertAlmostEqual(res["late_churn"].max(), 0.0, delta=1e-9)


    def test_decoupling_collapses_the_domain_scale(self):
    ...
    def test_diffusion_schedule_changes_the_outcome(self):
    ...
    def test_late_window_respects_late_frac(self):
