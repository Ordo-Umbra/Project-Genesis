...
class TestEmergence(unittest.TestCase):
    ...
    def test_identity_tpm_saturates_the_ceiling(self):
        """Perfectly determinate, perfectly non-degenerate -> EI = log2 N."""
        for n in (2, 3, 4, 8):
            out = effective_information(np.eye(n))
            self.assertAlmostEqual(out["ei"], np.log2(n), places=7)
            self.assertAlmostEqual(out["determinism"], 1.0, places=7)
            self.assertAlmostEqual(out["degeneracy"], 0.0, places=7)
            self.assertAlmostEqual(out["effectiveness"], 1.0, places=7)


    def test_uniform_tpm_has_zero_effective_information(self):
        """A system that ignores what you do to it scores zero."""
        out = effective_information(np.full((4, 4), 0.25))
        self.assertAlmostEqual(out["ei"], 0.0, places=7)


    def test_fully_degenerate_tpm_scores_zero_despite_being_deterministic(self):
        ...
        self.assertAlmostEqual(out["determinism"], 1.0, places=7)
        self.assertAlmostEqual(out["degeneracy"], 1.0, places=7)
        self.assertAlmostEqual(out["ei"], 0.0, delta=1e-12)


    def test_effectiveness_is_determinism_minus_degeneracy(self):
        rng = np.random.default_rng(0)
        tpm = rng.random((5, 5))
        tpm /= tpm.sum(axis=1, keepdims=True)
        out = effective_information(tpm)
        self.assertAlmostEqual(out["effectiveness"],
                               out["determinism"] - out["degeneracy"], places=7)


    def test_ei_never_exceeds_the_ceiling(self):
    ...
    def test_ei_is_non_negative_for_stochastic_matrices(self):
    ...
    def test_partial_noise_sits_between_the_limits(self):
        eps = 0.2
    ...
    def test_unnormalised_rows_are_normalised(self):
        a = effective_information(np.eye(3) * 7.0)
        self.assertAlmostEqual(a["ei"], np.log2(3), places=7)


    def test_non_square_input_is_rejected(self):
        with self.assertRaises(ValueError):
            effective_information(np.ones((2, 3)))


    def test_single_state_is_degenerate_by_definition(self):
    ...
    def test_block_slices_are_centred_and_the_right_size(self):
    ...
    def test_block_of_full_size_covers_everything(self):
    ...
    def test_majority_sector_picks_the_dominant_component(self):
    ...
    def test_intervention_tpm_rows_are_probability_distributions(self):
    ...
    def test_intervention_actually_overwrites_the_block(self):
    ...
    def test_a_whole_lattice_intervention_is_maximally_effective(self):
        ...
        self.assertAlmostEqual(effective_information(tpm)["ei"], np.log2(3), places=7)


    def test_backgrounds_are_independent_of_each_other(self):
    ...
    def test_emergence_profile_covers_every_requested_scale(self):
    ...
    def test_larger_blocks_are_at_least_as_effective_as_single_sites(self):
