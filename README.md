...
project_genesis/
  ...
  condensation.py      The condensation instruments: grow_defect_gas (spin defects from noise), amp_reaction_force (the detuning's third-law force on a mass — raw clumps, envelope-normalised is selective to the topological core), condensation_run (masses + a co-evolving defect gas)
...
**Honest scope:** this is **partial** self-assembly — a *controlled excess* of occupancy over chance (about ⅓ the wells at λ = 1), not yet wall-to-wall condensation. The transport window is bounded above (λ ≳ 1.5 is numerically unstable at dt = 0.1, and a very mobile core can annihilate before it traps), and a fraction of captures can be spurious near-well features rather than clean ½'s (seed-dependent, 0 to ~1/6 across runs). This is the *field-level* transport — the gas is mobilized and trapped by the wells' amplitude minima; combining it with the mass-side selective reaction force (the boundary's K3) into a single self-assembling molecule is the next rung. 2D, one lattice, several seeds, the derived floor spacing.
