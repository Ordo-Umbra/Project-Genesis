...
project_genesis/
  ...
  nematic_spinor.py    Half-integer spin: a nematic director field (n̂≡−n̂, RP¹) whose defects are ±½ disclinations — disclination_strength (s=q/2), director_holonomy (the 4π double cover: a 2π loop flips the director by −1, 4π restores it) — the topological realisation of a spinor (the SU(2) double cover of SO(3))
...
**Honest scope:** the statistics are **topological** (the far-field double-cover class of the composite), not quantum spin–statistics — no anticommutation, no Pauli principle between identical composites; that frontier stands. The constituent count n is put in by hand (n wells) — no dynamical confinement mechanism selects 2 or 3 (the N⋆ = 3 sector story is the standing suggestion, not wired in here). n = 3, 4 are static-well + evolving-field runs (the molecule dynamics is 2-body). 2D, one lattice, one operating point (the derived floor spacing), all-+½ constituents (the ½/−½ channel annihilates, per `n3_spinor`).
