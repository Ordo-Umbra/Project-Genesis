...
tests/                 322 checks across the engine, instruments, and physics
...
**Honest scope:** κ here is the diagnostic proxy `1/(1+load)`, not the dynamical capacity field; the weight w is a free dial, so the sharp claim is the *structure* (ΔC peaks at T_c; the optimum crosses T_c through a finite w-window), not any particular w value. One coupling point, L ∈ {32, 48}.
