...
tests/                 316 checks across the engine, instruments, and physics
...
- **The 3-D prediction is probed.** The Potts class predicts the 3-D transition is *first order*. On L ∈ {8, 10, 12, 16}: the hot/cold hysteresis window (T ∈ [0.10, 0.14]) persists at every size where a continuous transition would shrink it, and the Binder minima deepen monotonically with L, touching negative at L = 16 (−0.03 ± 0.15). Verdict: **partial first-order signatures — consistent with the weakly-first-order 3-D Potts transition, not conclusive at these sizes** (hot-start thermalisation in 3-D is slow and can mimic hysteresis; the window survived 600 compound sweeps).

Reproduce with `python experiments/n3_potts_transition.py`, `n3_potts_nu.py`, `n3_potts_3d.py` (≈ 25 minutes each; `--quick` for smoke runs).
